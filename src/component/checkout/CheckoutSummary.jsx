import { formatRupiah } from "../CartItem";
import { getImageProducts } from "../utils/image";

export default function CheckoutSummary({ order }) {
  // Pastikan order memiliki items
  const items = order.items || [];
  console.log(order)
  const subtotal = order.subtotal || 0;
  const total = order.subtotal + order?.shipping_price || 0;
  const shippingPrice = order.shipping_price

  return (
    <div className="grid gap-4 p-5 bg-white border border-black/10 rounded-xl">
      <h3 className="font-medium text-lg text-[#111827]">Ringkasan Pesanan</h3>

      <div className="grid gap-3">
        {items.map((item) => (
          <div key={item.id_product} className="flex items-center gap-2">
            <img src={getImageProducts(item.primary_img) || '/img/placeholder.png'} alt={item.product_name_snapshot} className="w-10 h-10 object-cover rounded-lg" />
            <div>
              <h4 className="font-normal text-xs text-[#6B7280] line-clamp-1">{item.product_name_snapshot}</h4>
            </div>
            <span className="ml-auto text-xs font-normal text-[#111827]">×{item.qty}</span>
          </div>
        ))}
      </div>

      <div className="grid pt-3 gap-2 border-t border-black/10">
        <div className="flex justify-between text-sm font-normal text-[#6B7280]">
          <span>Subtotal</span>
          <span>{formatRupiah(subtotal)}</span>
        </div>
        {shippingPrice && (
        <div className="flex justify-between text-sm font-normal text-[#6B7280]">
          <span>Ongkir</span>
          <span className="text-[#00A63E]">{shippingPrice}</span>
        </div>
        )}
        <div className="flex justify-between border-t border-black/10 pt-2 text-sm font-semibold text-[#1a202c]">
          <span>Total</span>
          <span className="text-green-600">{formatRupiah(total)}</span>
        </div>
      </div>

      <p className="text-center text-xs font-normal text-[#6B7280]">🛡️ Pembayaran aman dan terenkripsi</p>
    </div>
  );
}